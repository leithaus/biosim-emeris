 ...
 */
public class MyDragController extends AbstractDragController {
...
//	    LogTool.debug("dragMove() start");
		...
		if (getBehaviorConstrainedToBoundaryPanel()) {
			desiredLeft = Math.max(
					0,
					Math.min(desiredLeft, dropTargetClientWidth
		...
		DropController newDropController = getIntersectDropController(
...
//	    LogTool.debug("dragStart() start");
		...
		if (getBehaviorDragProxy()) {
			...
			container.setPixelSize(context.draggable.getOffsetWidth(),
					context.draggable.getOffsetHeight());
			context.boundaryPanel.add(container,
					currentDraggableLocation.getLeft(),
					currentDraggableLocation.getTop());
			...
			for (Widget widget : context.selectedWidgets) {
				widgetLocation.put(
						widget,
						new CoordinateLocation(widget.getAbsoluteLeft(), widget
								.getAbsoluteTop()));
			}
		...
		movablePanel.addStyleName(DragClientBundle.INSTANCE.css()
				.movablePanel());
		calcBoundaryOffset();
...
//		LogTool.debug("dragStart() end");
	...
	public void resetCache() {
		super.resetCache();
		dropControllerCollection.resetCache(getBoundaryPanel(), context);
	}
	...
	protected void restoreSelectedWidgetsLocation() {
		for (Widget widget : context.selectedWidgets) {
			...
			if (info.initialDraggableParent instanceof AbsolutePanel) {
				...
				((InsertPanel) info.initialDraggableParent).insert(widget,
	...
	protected void restoreSelectedWidgetsStyle() {
		for (Widget widget : context.selectedWidgets) {
			...
			widget.getElement().getStyle()
					.setProperty("margin", info.initialDraggableMargin);
		}
	}
...
//		GWT.log(msg);
	...
	private DropController getIntersectDropController(int x, int y) {
		...
		if (dropController != null) {
			dropController.toString();
		}
		dropController = dropController != null ? dropController
				: boundaryDropController;
		// GWT.log("" + x + ", " + y + " -- " + dropController);
