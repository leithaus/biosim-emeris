...
//  lazy val socketServerUrl = new URI("ws://127.0.0.1:8090/websocket")
  lazy val clientUriPathPart = "blah/blah/" +  UUID.randomUUID.toString
  lazy val clientUri = "websocket://" + clientUriPathPart

  lazy val socketServerUrl = {
    val uri = new URI("ws://127.0.0.1:8080/websocket/" + clientUriPathPart)
    println("server url is " + uri)
    uri
  }
//  lazy val socketServerUrl = {
//    val uri = new URI("ws://127.0.0.1:8080/websocket?uri=bob")
//    println("server url is " + uri)
//    uri
//  }
